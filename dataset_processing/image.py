...
class DepthImage(Image):
    ...
    def normalise(self):
        # self.img = np.clip((self.img - self.img.mean()) / 1000.0, -1, 1)  # 0216
        self.img = (self.img - self.img.min()) / np.float32(self.img.max() - self.img.min())

        # self.img = np.clip(np.true_divide((self.img/1000.0 - self.img.mean()/1000.0), np.std(self.img/1000.0)), -1, 1.1)
