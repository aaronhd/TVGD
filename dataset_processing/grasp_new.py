...
class GraspRectangle:
    ...
    def iou(self, gr, angle_threshold=np.pi/6):
        ...
        """
        # print("[DEBUG grasp_new]", self.angle, gr.angle)
        if abs((self.angle - gr.angle + np.pi/2) % np.pi - 1*np.pi/2) > angle_threshold:
            print('over 30 degree',  abs((self.angle - gr.angle + np.pi/2) % np.pi - 1*np.pi/2))
            return 0
...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1):
    ...
    """
    local_max = peak_local_max(q_img, min_distance=20, threshold_abs=0.1, num_peaks=no_grasps)  #15 0.1
    grasps = []
    for grasp_point_array in local_max:
        grasp_point = tuple(grasp_point_array)
        print('[DEBUG:grasp_new]', q_img[grasp_point])

        grasp_angle = ang_img[grasp_point]

        g = Grasp(grasp_point, grasp_angle)
        # print('[DEBUG grasp_new]', width_img[grasp_point])
        if width_img is not None:
            g.length = width_img[grasp_point]*241.0*1.4  #150*1.1; for RGBF model minmax normal 241 max width, we use 0.7 original wifth value inverse value is 1.4
            # g.width = g.length/2
            g.width = g.length*0.7  #0.7
